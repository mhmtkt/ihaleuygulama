...
def initialize_firebase():
    if not firebase_admin._apps:
        cred_dict = json.loads(st.secrets["firebase"]["firebase_service_account"])
...
def get_profile_info():
    ...
    garage_level = st.number_input("Garaj Seviyesi", min_value=1, max_value=100, value=profil.get("garage_level", 1))
    vehicle_count = st.number_input("Araç Sayısı", min_value=0, max_value=100, value=profil.get("vehicle_count", 0))
    trailer_count = st.number_input("Dorse Sayısı", min_value=0, max_value=100, value=profil.get("trailer_count", 0))
    ...
    if st.button("Profil Kaydet"):
        if vehicle_count > 0 and any(not n.strip() for n in yeni_vehicle_names):
            st.error("Araç isimlerini boş bırakmayın.")
            return
        yeni_profil = {
...
def ihale_girisi():
    ...
    ihale_bedeli = st.number_input("İhale Bedeli ($)", min_value=0.0, step=0.01)
    urun_birim_maliyeti = st.number_input("Birim Ürün Maliyeti ($)", min_value=0.0, step=0.01)
    urun_sayisi = st.number_input("Ürün Sayısı", min_value=0)

    if st.button("İhale Kaydet"):
        if not ihale_turu.strip():
...
def operasyonel_giderler():
    ...
    if kategori == "Garaj Bakımı":
        tutar = st.number_input("Garaj Bakım Tutarı ($)", min_value=0.0, step=0.01)
        ...
        maliyet = st.number_input("Yükseltme Maliyeti ($)", min_value=0.0, step=0.01)
        gider_detay["yeni_seviye"] = yeni_seviye
        gider_detay["tutar"] = maliyet
    elif kategori == "Maaş Ödemesi":
        sofor_adi = st.text_input("Şoför Adı")
        maas = st.number_input("Maaş Tutarı ($)", min_value=0.0, step=0.01)
        ...
        bakim_tutari = st.number_input("Bakım Maliyeti ($)", min_value=0.0, step=0.01)
        ...
        maliyet = st.number_input("Araç Maliyeti ($)", min_value=0.0, step=0.01)
        ...
        satis_tutari = st.number_input("Satış Tutarı ($)", min_value=0.0, step=0.01)
        gider_detay["arac_adi"] = arac_secimi
        gider_detay["tutar"] = -satis_tutari
    elif kategori == "Dorse Alımı":
        dorse_tipi = st.text_input("Dorse Tipi")
        maliyet = st.number_input("Dorse Maliyeti ($)", min_value=0.0, step=0.01)
        gider_detay["dorse_tipi"] = dorse_tipi
        gider_detay["tutar"] = maliyet
    elif kategori == "Emeklilik ve İşten Kovma":
        sofor_adi = st.text_input("Şoför Adı")
        tazminat = st.number_input("Tazminat Tutarı ($)", min_value=0.0, step=0.01)
        gider_detay["sofor_adi"] = sofor_adi
        ...
        if not arac_listesi:
            ...
            return
        arac_secimi = st.selectbox("Araç Seçin", arac_listesi)
        tutar = st.number_input("Yükseltme Bedeli ($)", min_value=0.0, step=0.01)
        gider_detay["arac_adi"] = arac_secimi
        gider_detay["tutar"] = tutar

    if st.button("Gider Kaydet"):
        if "tutar" not in gider_detay:
            st.error("Tutar bilgisi girin.")
            return
        ...
        kullanici_kaydet(st.session_state["logged_in_user"], user)
...
def rapor_goruntule():
    ...
    if df_giderler.empty:
        st.info("Henüz gider kaydınız yok.")
    else:
